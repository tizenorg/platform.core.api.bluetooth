...
tc_table_t tc_ipsp[] = {
	/* IPSP functions */
	{"BACK"								, BT_UNIT_TEST_FUNCTION_BACK},
	{"bt_le_ipsp_register[Server role]"					, BT_UNIT_TEST_FUNCTION_IPSP_REGISTER},
	{"bt_le_ipsp_unregister[Server role]"				, BT_UNIT_TEST_FUNCTION_IPSP_UNREGISTER},
	{"bt_le_ipsp_initialize[Server role]"					, BT_UNIT_TEST_FUNCTION_IPSP_INITIALIZE},
	{"bt_le_ipsp_deinitialize[Server role]"				, BT_UNIT_TEST_FUNCTION_IPSP_DEINITIALIZE},
	{"bt_le_ipsp_connect[Client role]"					, BT_UNIT_TEST_FUNCTION_IPSP_CONNECT},
	{"bt_le_ipsp_disconnect[Client role]"				, BT_UNIT_TEST_FUNCTION_IPSP_DISCONNECT},
	{"bt_le_ipsp_start_advertising[Server role]"					, BT_UNIT_TEST_FUNCTION_IPSP_START_ADVERTISING},
	{"bt_ipsp_set_connection_state_changed_cb[Client/Server role]"		, BT_UNIT_TEST_FUNCTION_IPSP_SET_CONNECTION_STATE_CHANGED_CB},
	...
	{"bt_ipsp_create_ipsp_app_server_socket[Server App role]"		, BT_UNIT_TEST_FUNCTION_IPSP_CREATE_APP_SERVER_SOCKET},
	{"bt_ipsp_connect_with_ipsp_app_server_socket[Client App role]"	, BT_UNIT_TEST_FUNCTION_IPSP_CONNECT_WITH_APP_SERVER_SOCKET},
	{"bt_ipsp_send_ipv6_application_data[Client/Server App role][Max 255 bytes]"		, BT_UNIT_TEST_FUNCTION_IPSP_SEND_IPV6_APP_DATA},
	{"bt_ipsp_recv_ipv6_application_data[Client/Server App role]"		, BT_UNIT_TEST_FUNCTION_IPSP_RECV_IPV6_APP_DATA},
	{"bt_ipsp_close_socket[Client/Server App role]"		, BT_UNIT_TEST_FUNCTION_IPSP_CLOSE_SOCKET},
	{"Select this menu to set parameters and then select the function again."			, BT_UNIT_TEST_FUNCTION_ACTIVATE_FLAG_TO_SET_PARAMETERS},
	{NULL					, 0x0000},
};
...
void __bt_le_ipsp_connection_bt_iface_info_cb(int result,
		const char *remote_address, const char *iface_name,
		void *user_data)
{
	TC_PRT("__bt_le_ipsp_connection_bt_iface_info_cb: called");
	TC_PRT("result: %s", __bt_get_error_message(result));
...
int test_input_callback(void *data)
{
	...
	case BT_UNIT_TEST_TABLE_IPSP: {
		...
		case BT_UNIT_TEST_FUNCTION_IPSP_SET_CONNECTION_BT_IFACE_INFO_CB:
			ret = _bt_le_ipsp_set_connection_bt_iface_info_cb(__bt_le_ipsp_connection_bt_iface_info_cb, NULL);
			TC_PRT("returns %s\n", __bt_get_error_message(ret));
			break;
