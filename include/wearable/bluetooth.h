 ...
 * @retval #BT_ERROR_NOT_SUPPORTED  Not supported
 *
 * @pre The Bluetooth pbap service must be initialized with bt_pbap_init().
 * @see bt_pbap_init()
 */
int bt_pbap_set_connection_state_changed_cb(bt_pbap_connection_state_changed_cb callback, void *user_data);
 ...
 * @see bt_pbap_init()
 */
int bt_pbap_unset_connection_state_changed_cb(void);
 ...
 *
 * @pre The state of the local Bluetooth adapter must be #BT_ADAPTER_ENABLED.
 * @pre The PBAP client must be initialized with bt_pbap_init().
 * @pre The local device must be bonded with the remote device by bt_device_create_bond().
 * @post bt_pbap_connection_state_changed_cb() will be invoked.
 * @see bt_pbap_disconnect()
 * @see bt_pbap_set_connection_state_changed_cb()
 */
int bt_pbap_connect(const char *address);
 ...
 * @pre The state of the local Bluetooth adapter must be #BT_ADAPTER_ENABLED.
 * @pre The PBAP client must be initialized with bt_pbap_init().
 * @pre PBAP connection must be created with bt_pbap_connect().
 * @post bt_pbap_connection_state_changed_cb() will be invoked.
 * @see bt_pbap_connect()
 * @see bt_pbap_set_connection_state_changed_cb()
 */
int bt_pbap_disconnect(const char *address);
 ...
 * @param[in] fields Fields of vCard to be fetched. Enable to set this value using 'or' operation using bt_pbap_field_e values
...
/**
 * @ingroup CAPI_NETWORK_BLUETOOTH_PBAP_CLIENT_MODULE
 * @brief Gets contact and call log information as the string type list from PBAP server, asynchronously.
 * @since_tizen 3.0
 * @privlevel public
 * @privilege http://tizen.org/privilege/bluetooth
 *
 ...
 * @param[in] source Source of phone book (Phone/SIM)
 * @param[in] folder_type Type of folder
 * @param[in] index The handle index of vCard to be fetched. We can know this value in the index of bt_pbap_vcard_info_s.
 ...
 * @pre The PBAP client must be initialized with bt_pbap_init().
 * @pre PBAP connection must be created with bt_pbap_connect().
 * @pre The vcard information must be generated with bt_pbap_get_list() or bt_pbap_search_phone_book().
 * @see bt_pbap_connect()
 * @see bt_pbap_get_list()
 * @see bt_pbap_search_phone_book()
 */
...
/**
 * @ingroup CAPI_NETWORK_BLUETOOTH_PBAP_CLIENT_MODULE
 * @brief Gets searched contact and call log information as the string type list from PBAP server, asynchronously.
 * @since_tizen 3.0
 * @privlevel public
 * @privilege http://tizen.org/privilege/bluetooth
 *
