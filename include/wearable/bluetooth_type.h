 ...
 * @param[in] enabled The PBAP connection status (@c true = enabled, @c false = disabled)
 * @param[in] user_data The user data passed from the callback registration function
 * @see bt_pbap_connect()
...
typedef void (*bt_pbap_phone_book_get_cb)(const char *remote_address,
		const char *vcf_file, bool successful, void *user_data);
 ...
 *
 * @param[in] remote_address The remote device address (@a remote_address is valid only inside this function. To use outside the callback, make a copy. @a remote_address should not be freed.)
 * @param[in] vcards List of vCards (@a vcards is valid only inside this function. To use outside the callback, make a copy. @a vcards should not be freed.)
...
typedef void (*bt_pbap_list_vcards_cb)(const char *remote_address,
		const char **vcards, int count, bool successful, void *user_data);

/**
 * @ingroup CAPI_NETWORK_BLUETOOTH_PBAP_CLIENT_MODULE
 * @brief  Called when PBAP Get vCard completes.
 * @details The received phone book file will be saved in the platform downloads folder.
 * @since_tizen 3.0
 *
 * @param[in] remote_address The remote device address (@a remote_address is valid only inside this function. To use outside the callback, make a copy. @a remote_address should not be freed.)
...
/**
 * @ingroup CAPI_NETWORK_BLUETOOTH_PBAP_CLIENT_MODULE
 * @brief  Called when PBAP Phonebook Search completes.
 * @since_tizen 3.0
 *
 * @param[in] remote_address The remote device address (@a remote_address is valid only inside this function. To use outside the callback, make a copy. @a remote_address should not be freed.)
 * @param[in] vcards List of vCards (@a vcards is valid only inside this function. To use outside the callback, make a copy. @a vcards should not be freed.)
 * @param[in] count Number of contacts in the list
 * @param[in] status The searching status (@c true = Successful, @c false = Unsuccessful)
 * @param[in] user_data The user data passed from the callback registration function
 * @see bt_pbap_phone_book_search()
 * @see bt_pbap_connect()
 * @see bt_pbap_disconnect()
 */
