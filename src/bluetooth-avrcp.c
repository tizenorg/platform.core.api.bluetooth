...
#ifdef NTB
	int error_code = BT_ERROR_NONE;
	error_code = ntb_bt_avrcp_target_notify_track(title, artist,
			album, genre, track_num, total_tracks, duration);
	return error_code;
#else
	int error;
	BT_CHECK_INIT_STATUS();
...
#ifdef NTB
	int error_code = BT_ERROR_NONE;
	error_code = ntb_bt_avrcp_set_equalizer_state_changed_cb(callback,
								 user_data);
	return error_code;
#else
	BT_CHECK_INIT_STATUS();
...
#ifdef NTB
	int error_code = BT_ERROR_NONE;
	error_code = ntb_bt_avrcp_set_repeat_mode_changed_cb(callback,
								user_data);
	return error_code;
#else
	BT_CHECK_INIT_STATUS();
...
#ifdef NTB
	int error_code = BT_ERROR_NONE;
	error_code = ntb_bt_avrcp_set_shuffle_mode_changed_cb(callback,
								user_data);
	return error_code;
#else
	BT_CHECK_INIT_STATUS();
