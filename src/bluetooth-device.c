...
#ifdef NTB
	int error_code = BT_ERROR_NONE;
	error_code = ntb_bt_device_create_bond_by_type(device_address, conn_type);
	return error_code;
#else
...
#ifdef NTB
	int error_code = BT_ERROR_NONE;
	error_code = ntb_bt_device_set_authorization(device_address, authorization);
	return error_code;
#else
	bluetooth_device_address_t addr_hex = { {0,} };
...
#ifdef NTB
	int error_code = BT_ERROR_NONE;
	error_code = ntb_bt_device_foreach_connected_profiles(remote_address, callback, user_data);
	return error_code;
#else
	bluetooth_device_address_t addr_hex = { {0,} };
...
#ifdef NTB
	int error_code = BT_ERROR_NONE;
	error_code = ntb_bt_device_set_authorization_changed_cb(callback, user_data);
	return error_code;
#else
	BT_CHECK_INIT_STATUS();
	BT_CHECK_INPUT_PARAMETER(callback);
...
#ifdef NTB
	int error_code = BT_ERROR_NONE;
	error_code = ntb_bt_device_set_connection_state_changed_cb(callback, user_data);
	return error_code;
#else
	BT_CHECK_INIT_STATUS();
	BT_CHECK_INPUT_PARAMETER(callback);
...
#ifdef NTB
	int error_code = BT_ERROR_NONE;
	error_code = ntb_bt_device_get_service_mask_from_uuid_list(uuids, no_of_service, service_mask_list);
	return error_code;
#else
	return BT_ERROR_NOT_SUPPORTED;
#endif
