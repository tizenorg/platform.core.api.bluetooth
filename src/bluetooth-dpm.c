...
int bt_dpm_set_allow_bluetooth_mode(bt_dpm_allow_e value)
{
	...
	error_code = _bt_dpm_get_error_code(bluetooth_dpm_set_allow_bt_mode(dpm_value));
...
int bt_dpm_get_allow_bluetooth_mode(bt_dpm_allow_e *value)
{
	...
	error_code = _bt_dpm_get_error_code(bluetooth_dpm_get_allow_bt_mode(&dpm_value));
	if (error_code != BT_ERROR_NONE)
		BT_ERR("%s(0x%08x)", _bt_convert_error_to_string(error_code), error_code);
	*value = dpm_value;
