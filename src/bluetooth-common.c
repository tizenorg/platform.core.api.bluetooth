...
/* LCOV_EXCL_STOP */
	...
	case BLUETOOTH_PBAP_PHONEBOOK_PULL: {
		...
		cb(device_addr, (const char *)pb_pull->vcf_file, (bool)pb_pull->success, user_data);
	...
	case BLUETOOTH_PBAP_VCARD_LIST: {
		...
		cb(device_addr, (const char **)vc_list->vcards, vc_list->length, vc_list->success, user_data);
	...
	case BLUETOOTH_PBAP_VCARD_PULL: {
		...
		cb(device_addr, (const char *)vc_pull->vcf_file, (bool)vc_pull->success, user_data);
	...
	case BLUETOOTH_PBAP_PHONEBOOK_SEARCH: {
		...
		_bt_convert_address_to_string(&device_addr, &vc_list->btaddr);
		cb(device_addr, (const char **)vc_list->vcards, vc_list->length, (bool)vc_list->success, user_data);
