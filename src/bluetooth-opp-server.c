			...
			void *user_data)
{
	...
	error_code = ntb_bt_opp_server_initialize(destination,
					push_requested_cb, user_data);
	return error_code;
#else
	int error_code = BT_ERROR_NONE;
...
#ifdef NTB
	int error_code = BT_ERROR_NONE;
	error_code = ntb_bt_opp_server_initialize_by_connection_request(
			destination, connection_requested_cb, user_data);
	return error_code;
#else
	int error_code = BT_ERROR_NONE;
...
#ifdef NTB
	int error_code = BT_ERROR_NONE;
	error_code = ntb_bt_opp_server_accept(progress_cb,
				finished_cb, name, user_data, transfer_id);
	return error_code;
#else
	int error_code = BT_ERROR_NONE;
