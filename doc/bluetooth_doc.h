 ...
 * @brief Bluetooth PBAP (Phone Book Access Profile) API provides functions for managing connections to a phone book server and getting phone book data.
 * @ingroup CAPI_NETWORK_BLUETOOTH_MODULE
 *
 ...
 * In PBAP Profile, there are two roles - @a Server and @a Client.
 * The @a Server is a device that has a phone book data. The @a Client is a device that requests the phone book data from the @a Server.
 ...
 *
 * You can check if a device supports features related to this API by using @ref CAPI_SYSTEM_SY STEM_INFO_MODULE, thereby controlling your application's actions.\n
 *
 * To ensure your application is only running on the device with specific features, please define the features in your manifest file using the manifest editor in the SDK.\n
 *
